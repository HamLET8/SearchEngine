...
void EventLoop::handlePendingCbs()
{
    ...
    //worker : _loopPtr->setPendingCallBack(std::bind(&TcpConnection::send, this, msg));
    for(auto& sendTask : pcbList)
    {
        sendTask();   
    }
}
...
void EventLoop::handleOldConnection(int clientFd) //handle search and close
{
    auto conn_it = _connMap.find(clientFd);

    if(conn_it != _connMap.end())
    {
        logInfo("find client fd %d in connection map", clientFd);
        if(conn_it->second->isClosed()) //close
        {
            conn_it->second->handleCloseCallBack();

            delEpollFd(clientFd);
            _connMap.erase(conn_it);
        }else{ //read and search
            conn_it->second->handleMessageCallBack();
            
            /* Expand the message callback function:
...
void EventLoop::waitEpoll()
{
    ...
    if(readyNum > 0)
    {
        ...
        for(int idx = 0; idx < readyNum ; ++idx)
        {
            ...
            {
                //search task have been done, create a send(msg) task and return.  
                if(_eventList[idx].events & EPOLLIN)
                {
                    logInfo("eventfd coming");
                    readCounter();
                    handlePendingCbs(); //handle send msg task
                }   
            }else{
                //recived message from client, create a MyTask::process task and push into TaskQueue
                if(_eventList[idx].events & EPOLLIN)
                {   
